...
async def run_profile(target):
    """Async wrapper for profile command"""
    client = get_client()
    async with client:
...
async def run_members(target, max_members):
    """Async wrapper for members command"""
    await utils.init_db()
    client = get_client()
    async with client:
        await start_client(client)
...
async def run_messages(target, limit):
    """Async wrapper for messages command"""
    await utils.init_db()
    client = get_client()
    async with client:
        await start_client(client)
...
async def run_media(target, limit, output_dir):
    """Async wrapper for media command"""
    await utils.init_db()
    client = get_client()
    async with client:
        await start_client(client)
